...
class VP_Option_Control_Set
{
	...
	public function save($option_key)
	{
		...
		if(update_option($option_key, $opt))
		{
			...
			$result['message'] = __('Saving successful', 'easy-image-collage');
		...
		else
		{
			...
			if($changed)
			{
				...
				$result['message'] = __('Saving failed', 'easy-image-collage');
			}
			else
			{
				$result['status']  = true;
				$result['code']    = self::SAVE_NOCHANGES;
				$result['message'] = __('No changes made', 'easy-image-collage');
			}
		}
